...
F32 compensationCoefficient = 1.0; // 温度校准系数
...
{
	ADC_ConvertedValueLocal[0] = (float)Value / 4096 * 3.3; // AD转换
	...
	// 			(int)(TDS_value * 100) % 100 / 10 + (int)(TDS_value * 100) % 10; // 获取数据
	return TDS_value;
}
// 返回湿度
F32 Offset = 0.00; // deviation compensate
F32 phValue = 0;
float Get_Adc_Average(u16 Value, u8 times) // 获取土壤湿度数据并返回给主函数
{
	//	ADC =(float) ADC_ConvertedValue/4096*3.3;
	phValue = (float)Value / 4096 * 3.3; // AD转换
